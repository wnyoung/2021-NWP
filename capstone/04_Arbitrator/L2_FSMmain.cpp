...
void L2_initFSM(uint8_t myId, uint8_t destId)
{
    ...
    L2_validityCheck_ID();      // myL2ID == destL2ID 체크

    L2_LLI_initLowLayer(myL2ID);                    // 아마 아랫단에 자기의 id를 저장하는 듯...?
    L3_LLI_setDataReqFunc(L2_LLI_handleDataReq);    // 아마 Data REQ 함수를 setting하는 듯...?
}
    ...
    switch (main_state)
    {
        case L2STATE_IDLE: //IDLE state description
            ...
            if (L2_event_checkEventFlag(L2_event_dataRcvd)) //if data reception event happens
            {
                ...
                L3_LLI_dataInd(L2_msg_getWord(dataPtr), size-L2_MSG_OFFSET_DATA, L2_LLI_getSnr(), L2_LLI_getRssi());    // 여기서 L3_event_msgRcvd SETTING
