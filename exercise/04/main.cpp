...
int main(void) {
    ...
    while (1)
    {
        ...
        case MAINSTATE_TX: //TX state description
            ...
            {
                if (arqTimer_getTimerStatus() == 1 ||      //타이머로 transition 구분-> 내 PDU에 대한 retransmission 고려(= 타이머가 돌고 있거나 만료) ack state로
            ...
            {
                main_state = MAINSTATE_ACK;
                arqTimer_startTimer();      //타이머를 start

                arqEvent_clearEventFlag(arqEvent_dataTxDone);
            }
            //그외 arqEvent_ackRcvd, arqEvent_dataRcvd, arqEvent_dataToSend 이벤트는 딜레이시킴(다른 state에서 처리)
